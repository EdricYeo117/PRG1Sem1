...
def calculate_card_value(card_value, time_of_entry):
    ...
    if time_of_entry < 12.00 or time_of_entry >= 20.00:
        erp_rate = 0
    elif time_of_entry < 17.30 or time_of_entry >= 19.55:
        erp_rate = 0.50
    elif time_of_entry < 17.35 or time_of_entry >= 19.00:
        erp_rate = 1.00
    elif time_of_entry < 18.00 or time_of_entry >= 18.55:
        erp_rate = 1.50
    else:
        erp_rate = 2.00
    ...
    if erp_rate > 0.0:#Check if there's erp charges
        if card_value > erp_rate:  #Check if there is enough balance to pay ERP charge
            card_value -= erp_rate   #Modify to calculate new value in cash card
            successful_deduction = True   #Set the value for successful_deduction
        else:
            print("You do not have enough balance in your card to pay for the ERP charge. /n The amount is not deducted, you need to pay a fine for this.") #Modify to display insufficent balance in cash card
...
if successful_deduction == True:
    print("The amount is deducted successfully")

#Print final card value
print("Your card value is ${:.2f}".format(card_value))
