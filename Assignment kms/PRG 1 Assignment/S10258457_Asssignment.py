...
def total_basement_carpark():
    ...
    basement_carpark_list = list(filter(lambda items: items["Carpark Type"] == "COVERED CAR PARK", data_list))
...
def read_carpark_availability(filepath):
    ...
    while 1:
        try:
            #no need to file.close because this automatically closes
            with open(filepath, "r") as file:
                ...
                print(cp_availability[0])
                file.close()#this is redundant but I'm paranoid
            break
...
def carpark_location_check():
    '''    ---- Option 8 ----
    Function to print carparks at a given location
...
cp_availability = []
print('** carpark_list[782]', data_list[782])  
