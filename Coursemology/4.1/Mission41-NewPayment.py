...
def cal_new_payment(age, amount):
    if age >= 60:#Check if user is eligible for discount
        ...
        #Calculate new payment amount, round up to integer
        print("Original amount to pay is: $ {}".format(amount)) #Modify to display original payment amount
        print("New amount to pay is: $ {}".format(new_amount)) #Modify to display new payment amount
        return new_amount #Do not remove this line that returns the new amount
    else:
        print("Not eligible for discount") #Modify to display message indicating not entitled to discount
...
#print(new_amount) #Modify to display the amount obtained from function call
